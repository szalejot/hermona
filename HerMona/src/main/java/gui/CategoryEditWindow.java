...
public class CategoryEditWindow extends JFrame {
	...
	public CategoryEditWindow() {
		super("Edytuj kategorie");

		setSize(300, 300);
		setResizable(true);
		setLocationRelativeTo(null);
		
		Vector<Kategoria> katVec= new Vector<Kategoria>(dbUtil.getCategories());
		tableModel = new InteractiveTableModel(columnNames, katVec);
        tableModel.addTableModelListener(new CategoryEditWindow.InteractiveTableModelListener());
        table = new JTable();
	...
	private class InteractiveTableModel extends AbstractTableModel {
		...
		public Kategoria getObjectAt(int row) {
